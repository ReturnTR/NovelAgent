...
class AgentCard(BaseModel):
    ...
    created_at: str = Field(default_factory=lambda: datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"))  # 创建时间
...
class EventType(str, Enum):
    ...
    - USER_MESSAGE: Web Console 发起的用户消息
    """
...
class A2AEvent(BaseModel):
    ...
    timestamp: str = Field(default_factory=lambda: datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"))  # 时间戳
